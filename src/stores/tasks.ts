...
import { TaskStatus, type CreateTaskPayload, type Task, type UpdateTaskPayload } from '@/types/task'
...
export const useTasksStore = defineStore('tasks', () => {
  ...
  async function fetchByProject(projectId: number): Promise<void> {
    ...
    try {
      ...
      error.value = err instanceof AppError ? err.message : 'Не вдалося завантажити завдання'
    } finally {
   ...
   */
  async function moveTask(taskId: number, status: TaskStatus, newIndex: number): Promise<void> {
    ...
    if (previousStatus !== status) {
      ...
      const oldColumn = tasks.value.filter((t) => t.status === previousStatus).sort((a, b) => a.order - b.order)
      await Promise.all(oldColumn.map((t, index) => tasksApi.update(t.id, { order: index })))
    }
  }

  /** Reorders rows within the table (same status) via drag-and-drop. */
  async function reorderWithinStatus(status: TaskStatus, orderedIds: number[]): Promise<void> {
    orderedIds.forEach((id, index) => {
      const t = tasks.value.find((task) => task.id === id)
      if (t) t.order = index
    })
    await Promise.all(orderedIds.map((id, index) => tasksApi.update(id, { order: index })))
  }
